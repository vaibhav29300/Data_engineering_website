...
const HomePage: React.FC = () => {
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-white dark:bg-gray-900 text-gray-800 dark:text-white py-12">
      <motion.div 
      ...
      >
        <h1 className="text-5xl font-bold mb-4">Your Data Engineering Companion</h1>
        <p className="text-xl text-gray-600 dark:text-gray-300 mb-8">Everything you need to become a skilled Data Engineer.</p>
        <div className="space-x-4">
          ...
          <Link to="/roadmap"> {/* Pointing to roadmap as a general starting point */}
            <motion.button 
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
      ...
      <motion.div 
        ...
        className="bg-gray-50 dark:bg-gray-800 p-8 rounded-lg shadow-xl max-w-2xl text-center mb-12"
      >
        <h2 className="text-3xl font-semibold mb-4">Who is a Data Engineer?</h2>
      ...
      >
        <h2 className="text-3xl font-semibold text-center mb-6">Quick Links</h2>
          ...
          {[ "Roadmap", "Resources", "Tools", "Projects", "Resume Tips" ].map((item) => (
            <Link key={item} to={`/${item.toLowerCase().replace(' ', '-')}`}>
              <motion.div 
                ...
                className="bg-white dark:bg-gray-700 p-6 rounded-lg shadow-lg text-center cursor-pointer h-full flex items-center justify-center"
              >
                <h3 className="text-xl font-medium text-blue-600 dark:text-blue-400">{item}</h3>
              </motion.div>
            </Link>
          ))}
        </div>
